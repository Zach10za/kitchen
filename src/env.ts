export interface Env {
  ...
  ADMIN_TOKEN: string;
  GITHUB_TOKEN: string;
  /** Tavily search API key (tvly-…). Powers the kitchen bot's `web_search`
   *  function tool. If unset, search degrades gracefully and the model falls
   *  back to its own knowledge. Free tier (~1k searches/mo) covers single-user use. */
