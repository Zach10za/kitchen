...
/**
 * Durable tasks agent flow. Mirrors FinanceSteerWorkflow but targets TasksDO's
 * IO endpoints and the TASKS_TOOLS list. Each agent loop iteration is its own
 * step with retries — slow LLM calls don't take down the conversation.
 *
 * Steps:
 *   1. load-context     — system prompt + recent history from TasksDO
 ...
 */
export class TasksSteerWorkflow extends WorkflowEntrypoint<Env, TasksSteerParams> {
  async run(event: WorkflowEvent<TasksSteerParams>, step: WorkflowStep) {
    ...
    const stub = this.tasks();

    // The thread channel id doubles as our conversation-scope key.
    ...
    const ctx = await step.do('load-context', async () => {
      const res = await stub.fetch(
        `https://internal/workflow/tasks/load-context?thread_id=${encodeURIComponent(threadId)}`,
      );
    ...
    await step.do('save-user-turn', async () => {
      await stub.fetch('https://internal/workflow/tasks/save-turn', {
        method: 'POST',
    ...
    // record-usage is observability — never let it block delivery (matches
    // the workout-steer pattern; tasks was previously missing this guard).
    const turnTotals = await step.do('record-usage', async () => {
      try {
        const res = await stub.fetch('https://internal/workflow/tasks/record-usage', {
          method: 'POST',
          body: JSON.stringify({
            thread_id: threadId,
  ...
  private async runOneRound(messages: any[], threadId: string): Promise<RoundResult> {
    ...
    return runAgentRound({
      ...
      executeTool: async (name, args) => {
        const res = await stub.fetch('https://internal/workflow/tasks/exec-tool', {
          method: 'POST',
  ...
  private tasks() {
    return this.env.TASKS.get(this.env.TASKS.idFromName('default-household'));
  }
}
