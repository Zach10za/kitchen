...
export const FINANCE_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'cash_flow',
      description: 'Daily inflows vs outflows over a window, with inter-account transfers excluded (transfers between the user\'s own accounts — checking↔savings, credit-card payments — are detected as paired flows or by Category and dropped). Use for "what\'s my daily cash flow", "income vs spending by day", "am I cash-flow positive". Mirrors the Cash Flow tab + chart in the sheet.',
      parameters: {
        type: 'object',
        properties: {
          days: { type: 'integer', minimum: 1, maximum: 365, description: 'Lookback window. Default 30.' },
