...
- **cash_flow**: monthly inflows vs outflows. Excludes anything categorized **"Transfer"** (inter-account moves, auto-tagged) or **"Exclude"** (a manual escape hatch for one-offs that aren't transfers but distort the picture — e.g. a large one-time bonus). Mirrors the live Cash Flow tab + chart. To drop a row from cash flow, categorize it "Transfer" or "Exclude" — the sheet's formulas update instantly.
