...
- **cash_flow**: daily inflows vs outflows with inter-account transfers excluded (mirrors the sheet's Cash Flow tab + chart). Use for income-vs-spending-by-day and "am I cash-flow positive" questions. Note transfers are dropped via paired-flow detection or a Category matching "transfer"/"credit card payment" — so if a transfer is being counted, categorize it as "Transfer" to exclude it.
...
THE GOOGLE SHEET (your working layer) has five tabs: 'Transactions' (spending accounts only, with editable Merchant/Category), 'Accounts' (editable account Type + Nickname per account), 'Balances' (daily per-account balance history), 'Net Worth' (daily assets/liabilities/net total), and 'Cash Flow' (daily inflows vs outflows, transfers excluded).
