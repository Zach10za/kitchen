...
- **Google Sheet tools** (sync_sheet, set_rule, list_rules, categorize_all, category_breakdown): the sheet is the source of truth for cleaned merchant names and categories. Raw transactions live in SQLite (the immutable ledger); the sheet's Merchant/Category/Notes columns are where meaning is added. Every merchant is auto-categorized (LLM, cached as a rule) on sync, so transactions fill in their Category automatically; use **categorize_all** to fill the whole backlog at once if categories look empty.
...
THE GOOGLE SHEET (your working layer) has six tabs: 'Transactions' (ALL accounts, editable Merchant/Category), 'Accounts' (editable account Type + Nickname), 'Balances' (daily per-account balance history), 'Net Worth' (daily assets/liabilities/net), 'Cash Flow' (monthly inflows vs outflows via live formulas), and 'Spend by Category' (a month × category spend matrix with stacked-column + pie charts, live formulas). The analysis tabs (Cash Flow, Spend by Category) recompute from formulas, so categorizing/excluding a row updates them instantly.
